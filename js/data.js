const SBH = {
  ...
  defaultProducts: [
    ...
    { id:2,  name:"Snickerdoodle Dreams",           category:"Cookies", price:13.99, emoji:"🌀", desc:"Soft and pillowy cinnamon sugar cookies with perfectly crisp edges.",                               longDesc:"Rolled generously in cinnamon sugar and baked until soft in the center with slightly crisp crinkled edges.", bestSeller:false, status:"instock",  isNew:true,  featured:false, allergens:"Contains gluten, dairy, eggs" },
    { id:3,  name:"Double Fudge Brownies",          category:"Cookies", price:16.99, emoji:"🟫", desc:"Rich dense fudge brownies with a signature crinkly top.",                                          longDesc:"Two types of dark chocolate. Dense fudgy and intensely chocolatey with that classic shiny top.", bestSeller:true,  status:"instock",  isNew:false, featured:false, allergens:"Contains gluten, dairy, eggs" },
    { id:4,  name:"Strawberry Shortcake",           category:"Cakes",   price:42.99, emoji:"🍓", desc:"Layers of fluffy vanilla sponge fresh strawberries and whipped cream frosting.",                   longDesc:"Three layers of light vanilla sponge filled with fresh macerated strawberries and house made whipped cream.", bestSeller:true,  status:"instock",  isNew:false, featured:true,  allergens:"Contains gluten, dairy, eggs" },
    ...
    { id:11, name:"Chocolate Fudge Cake",           category:"Cakes",   price:48.99, emoji:"🎂", desc:"Three layers of decadent dark chocolate sponge with silky ganache frosting.",                      longDesc:"Three rich dark chocolate sponge layers filled and frosted with a velvety dark chocolate ganache.", bestSeller:false, status:"instock",  isNew:false, featured:false, allergens:"Contains gluten, dairy, eggs" },
    { id:12, name:"Banana Nut Bread",               category:"Bakery",  price:9.99,  emoji:"🍌", desc:"Perfectly moist warmly spiced banana bread with toasted walnuts.",                                 longDesc:"Made with overripe bananas for maximum sweetness and toasted walnuts throughout.", bestSeller:false, status:"instock",  isNew:false, featured:false, allergens:"Contains gluten, dairy, eggs, tree nuts" }
  ],
  ...
  /* ---- PRODUCTS ---- */
  getProducts() {
    try { const s = localStorage.getItem("sbh_products"); return s ? JSON.parse(s) : this.defaultProducts; } catch { return this.defaultProducts; }
  },
  ...
  addToCart(product, qty = 1) {
    ...
    if (idx > -1) { cart[idx].qty += qty; } else { cart.push({ id:product.id, name:product.name, price:product.price, emoji:product.emoji, qty }); }
  ...
  getCartTotal() {
    ...
    return this.getCart().reduce((s,item) => { const p = products.find(p => p.id === item.id); return s + (p ? p.price : item.price) * item.qty; }, 0);
  },
  updateCartBadge() {
  ...
  /* ---- SETTINGS ---- */
  getSettings() {
    try { const s = localStorage.getItem("sbh_settings"); return s ? {...this.defaultSettings, ...JSON.parse(s)} : {...this.defaultSettings}; } catch { return {...this.defaultSettings}; }
  },
  saveSettings(s) { try { localStorage.setItem("sbh_settings", JSON.stringify(s)); } catch {} },

  /* ---- ORDERS (real data only) ---- */
  ...
  getAdminUsers() {
    try {
      ...
      // Default master user on first load
  ...
  setAdminSession(user) { try { localStorage.setItem("sbh_admin_session", JSON.stringify({id:user.id, name:user.name, email:user.email, role:user.role})); } catch {} },
